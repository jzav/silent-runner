...
sr_test_init(DESCRIPTION "Replays a JSONL child payload ending in a comma without a newline in text, dual, and base64 presentations.")
...
foreach(presentation IN ITEMS default text text+base64 base64+text base64)
    message(STATUS "JSONL trailing-comma presentation: ${presentation}")
    ...
    set(presentation_args)
    set(expected_representation "${presentation}")
    if(presentation STREQUAL "default")
        set(expected_representation "text")
    else()
        list(APPEND presentation_args --jsonl-payload-presentation "${presentation}")
    endif()
    if(presentation STREQUAL "base64+text")
    ...
    if(header_end LESS 0)
        message(FATAL_ERROR "Missing parent stdout header for ${presentation}: [${r_STDOUT}]")
    endif()
    ...
    if(NOT header_type EQUAL 0 OR header_size LESS 0)
        message(FATAL_ERROR "Unexpected parent header for ${presentation}: [${parent_header}]")
    endif()
    ...
    if(NOT parent_payload STREQUAL "tail,")
        message(FATAL_ERROR "Expected exact parent payload [tail,] for ${presentation}, got [${parent_payload}]")
    endif()
    ...
    if(NOT record_count EQUAL 1)
        message(FATAL_ERROR "Expected one EOF-flushed JSONL event for ${presentation}, got ${record_count}")
    endif()
    ...
    if(NOT payloadType STREQUAL "ChildStdout"
        ...
        message(FATAL_ERROR "Unexpected JSONL metadata for ${presentation}: ${record}")
    endif()
    ...
    if(NOT payloadText STREQUAL expected_text OR NOT payloadBase64 STREQUAL expected_base64)
        message(FATAL_ERROR "Unexpected JSONL payload fields for ${presentation}: ${record}")
    endif()
endforeach()
